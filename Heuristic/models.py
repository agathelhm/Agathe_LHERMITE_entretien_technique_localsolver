...
def run_model(filename : str, model : str, method : str, ordering_periods : list) -> tuple:
    #pick_periods = 'all', n = 0, method = 'not_sequential'
    """
    input : 
    ...
    forbidden_periods = list(set(periods).difference(set(ordering_periods))) # calculate forbidden periods
    ordering_periods = list(set(ordering_periods) & set(periods))
    ordering_periods.sort()


    # fixed periods - modify parameter f
    if ordering_periods_exists and method in ['infinite_f']  :
        f_init = f.copy() 
        for t in forbidden_periods :
            f[0,t] = m.infinity
    ...
    if ordering_periods_exists and method in ['fix_y'] :
        if model in ['owmr', 'es', 'ses'] : # model with variable y :
            m.add_constraints((y[0,t] == 0 for t in forbidden_periods), names = 'periods')
        else : # model with variable y_0
            m.add_constraints((y_0[0,t] == 0 for t in forbidden_periods), names = 'periods')    
    ...
    #if model in ['mc']:
    #    print([y_0[0,t].solution_value for t in periods])

    return(m.objective_value,  m.solve_details.time)
